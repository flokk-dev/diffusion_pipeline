...
from src.backend.text_diffuser.utils import get_width, get_key_words, adjust_overlap_box, shrink_box, \
    adjust_font_size, \
    alphabet_dic
...
def process_caption(font_path, caption, keywords):
    ...
    caption_words = tokenizer([caption], truncation=True, max_length=77, return_length=True,
    ...
    for keyword in keywords:
        ...
        if keyword in caption_split:
            if keyword not in duplicate_dict:
                ...
                if duplicate_dict[keyword] + 1 < len(caption_split) and keyword in caption_split[
                                                                                   duplicate_dict[
                                                                                       keyword] + 1:]:
                    index = duplicate_dict[keyword] + caption_split[
                                                      duplicate_dict[keyword] + 1:].index(keyword)
    ...
    return caption, length_list, width_list, torch.from_numpy(info_array), words, torch.Tensor(
        state_list).long(), torch.Tensor(word_match_list).long(), torch.Tensor(boxes), boxes_length
...
def get_layout_from_prompt(prompt: str):
    ...
    print(f'{colored("[!]", "red")} Detected keywords: {keywords} from prompt {prompt}')

    text_embedding, mask = text_encoder(prompt)  # (1, 77 768) / (1, 77)

    # process all relevant info
    caption, length_list, width_list, target, words, state_list, word_match_list, boxes, boxes_length = process_caption(
        font_path, prompt, keywords)
    ...
    with torch.no_grad():
        for box_index in range(boxes_length):
            ...
            output, encoder_embedding = model(text_embedding, length_list, width_list, mask,
                                              state_list, word_match_list, target,
                                              right_shifted_boxes, train=False,
    ...
    for index, box in enumerate(return_boxes):
        ...
        for i, char in enumerate(text):
            ...
            segmentation_mask_draw.rectangle(shrink_box(char_box, scale_factor=0.9),
                                             fill=char_index)
