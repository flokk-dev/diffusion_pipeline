...
def segmentation_mask_visualization(font_path: str, segmentation_mask: np.array):
    ...
    for i in range(64):
        for j in range(64):
            if int(segmentation_mask[i][j]) == 0 or int(segmentation_mask[i][j]) - 1 >= len(
                    alphabet):
                continue
            else:
                d.text((j * 8, i * 8), alphabet[int(segmentation_mask[i][j]) - 1], font=font,
                       fill=(0, 255, 0))
    return blank
...
def make_caption_pil(font_path: str, captions: List[str]):
    ...
    for caption in captions:
        ...
        img = ImageOps.expand(img, border=(border_size, border_size, border_size, border_size),
                              fill=(127, 127, 127))
...
def combine_image(args, sub_output_dir: str, pred_image_list: List, image_pil: Image,
                  character_mask_pil: Image, character_mask_highlight_pil: Image,
...
def combine_image_gradio(args, sub_output_dir: str, pred_image_list: List, image_pil: Image,
                         character_mask_pil: Image, character_mask_highlight_pil: Image,
                         caption_pil_list: List):
    """
    This function combines all the outputs and useful inputs together.
